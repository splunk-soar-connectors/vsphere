...
class _Mirage:
    ...
    def __call__(self, *args, **kwargs):
        if issubclass(self.klass, ElementDeclaration):
            return self._reveal_element(*args, **kwargs) if self.__reveal else self._hide_element(*args, **kwargs)
        else:
            return self._reveal_type(*args, **kwargs) if self.__reveal else self._hide_type(*args, **kwargs)
...
from pysphere.ZSI.TC import Any, RegisterType, MetaTypeCode

class MetaTypeCodeAndSchemaInstanceType(MetaTypeCode, SchemaInstanceType):
    '''Common subclass of MetaTypeCode and SchemaInstanceType metaclasses.'''
    pass
